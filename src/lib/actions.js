...
export function checkForWinner(pazaak, online) {
  ...
  if (typeof winner !== "undefined") {
    ...
    if (online) firestore.updateRoomDocument(pazaak.roomCode, newPazaak)
 ...
 */
export function playSideDeckCard(pazaak, cardIndex, pid, online) {
 ...
 * Hit New Card
 */
export function hit(pazaak, pid, oid, online) {
  try {
    const newPazaak = pazaak
    const player = newPazaak.players[pid]
    ...
    // Set Stack And Store
    newPazaak.players[pid].stack[nextPlaceholder] = game.dealCard()
 ...
 */
export function stand(pazaak, pid, oid, online) {
  const newPazaak = pazaak

  // Player Stands On A Bust
  if (newPazaak.players[pid].score > 20) {
    // Reset Player Stack And Score
    newPazaak.players[pid].stack = game.initializeStack(false)
    ...
    newPazaak.players[oid].stack = game.initializeStack(true)
  ...
  if (!pazaak.standing.includes(pid) && !pazaak.standing.includes(oid)) {
    ...
    newPazaak.players[oid].stack[nextPlaceholder] = game.dealCard()
